...
function Selection({ open, handleClose, activity,setBatsman,setBatting,setBowler }) {
...
function Batting({ max, handle }) {
  ...
  const { team1, team2, batting, team1Players, team2Players,battedPlayers1,battedPlayers2,batsman1,batsman2 } = useSelector(
    (state) => state.scorecard
  );
  const isValidBatsman=(k)=>{
    if(batting===1){
      return batsman1.key===k||batsman2.key===k||battedPlayers1.find(el=>el.key===k)!==undefined||(batsmen.length === max &&!batsmen.find((el) => el.key ===k))
    }
    return batsman1.key===k||batsman2.key===k||battedPlayers2.find(el=>el.key===k)!==undefined||(batsmen.length === max &&!batsmen.find((el) => el.key === k))
  }
  return (
    <>
